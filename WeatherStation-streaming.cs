...
namespace CumulusMX
{
	internal abstract class WeatherStation
	{
				...
				if (File.Exists(logFile))
				{
					...
					try
					{
						if (rowsToAdd.Count > 0)
						{
							...
							RecentDataDb.InsertAll(rowsToAdd);
						}
					}
					catch (Exception e)
					{
		...
		*/
		...
		public async Task GetDayfile(string draw, int start, int length, TextWriter writer)
		{
			try
			{
				...
				if (!File.Exists(cumulus.DayFileName))
				{
					Cumulus.LogMessage($"GetDayFile: Error, dayfile file does not exist");
					return;
				}

				writer.Write("{\"draw\":" + draw);
				writer.Write(",\"data\":[");
				...
				using (StreamReader sr = File.OpenText(cumulus.DayFileName))
				{
					while ((line = await sr.ReadLineAsync()) != null)
					{
						if (lineNum < start || lineNum >= last)
						{
							lineNum++;
							continue;
						}
				...
				writer.Write($"],\"recordsTotal\":{lineNum},\"recordsFiltered\":{lineNum}}}");
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// Streams the log file lines to json format
		/// </summary>
		...
		/// <returns></returns>
		public async Task GetLogfile(string date, string draw, int start, int length, bool extra, TextWriter writer)
		{
			try
			{
				...
				writer.Write($"\"draw\":{draw},\"recordsTotal\":{lineNum},\"recordsFiltered\":{lineNum}}}");
			}
			catch (Exception ex)
			{
				Cumulus.LogMessage(ex.ToString());
