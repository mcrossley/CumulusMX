...
namespace CumulusMX
{
	internal abstract class WeatherStation
	{
		...
		public WeatherStation(Cumulus cumuls, bool extraStation = false)
		{
			...
			RecentWind = new TRecentWind[MaxWindRecent];
			...
			_ = Database.CreateTable<WindRecent>();
			_ = Database.Execute("create table if not exists WindRecentPointer (pntr INTEGER)");
		...
		private bool CheckSqliteDatabase(bool giveup)
		{
			...
			try
			{
				...
				var cmd = Database.CreateCommand("PRAGMA quick_check;");
			...
			if (!corrupt && !giveup && errorCount > 0)
			{
				...
				{
					cumulus.LogCriticalMessage("SQLite compress failed - " + ex.Message);
					corrupt = true;
				}
			}
			...
			if (corrupt)
			{
				cumulus.LogCriticalMessage("The SQLite database is corrupt!");
				cumulus.LogCriticalMessage("Please restore Cumulus MX data from a known good backup.");
				return false;
			}
			else
			{
		...
		{
			while (cumulus.MySqlFunction.FailedList.TryDequeue(out var _))
			{
				// do nothing
			};
			...
			{
				cumulus.MySqlFunction.FailedList.Enqueue(rec);
			}
		}
		...
		private void GetRainCounter()
		{
			...
			// If we do not have a midnight rain counter value from Today.ini, find it in the database
			if (initialiseMidnightRain)
			{
				bool midnightrainfound = false;

				cumulus.LogMessage("Finding midnight rain counter from database");
				try
				{
					var rec = Database.Query<IntervalData>("SELECT Timestamp, RainCounter FROM IntervalData WHERE Timestamp >= ? ORDER BY Timestamp LIMIT 1", cumulus.LastUpdateTime.Date.ToUnixTime());

					if (rec[0].RainCounter.HasValue)
					{
						// this is the first entry of a new day AND the new day is today
						midnightrainfound = true;
						cumulus.LogMessage($"Midnight rain counter found in the following entry: {rec[0].StationTime}, RainCounter = {rec[0].RainCounter}");
						...
						else if (Math.Round(RainCounterDayStart, cumulus.RainDPlaces) == Math.Round(raincount, cumulus.RainDPlaces))
						{
							cumulus.LogMessage($"raindaystart {RainCounterDayStart:F4} and midnight rain {raincount:F4} match within rounding error, setting midnight rain to rainday start value");
			...
			// If we do not have a rain day start counter value from Today.ini
			if (initialiseRainDayStart)
			{
				cumulus.LogMessage("Finding rain day start counter from database");
				try
				{
					...
					var rec = Database.Query<IntervalData>("SELECT Timestamp, RainCounter FROM IntervalData WHERE Timestamp >= ? ORDER BY Timestamp LIMIT 1", meteoDate.ToUnixTime());

					if (rec[0].RainCounter.HasValue)
					{
						// this is the last entry
						cumulus.LogMessage($"Current rain counter found in the following entry: {rec[0].StationTime}, RainCounter = {rec[0].RainCounter}");
						raincount = rec[0].RainCounter.Value;

						cumulus.LogMessage($"GetRainCounter: Rain counter found, setting existing rain counter {RainCounter:F4} to log file value {raincount:F4}");
						RainCounter = raincount;
			...
			{
				cumulus.LogMessage("Finding current rain counter from database");
				try
				{
					var rec = Database.Query<IntervalData>("SELECT Timestamp, RainCounter FROM IntervalData WHERE Timestamp >= ? ORDER BY Timestamp DESC LIMIT 1", cumulus.LastUpdateTime.Date.ToUnixTime());
		...
		public void GetRainFallTotals()
		{
			...
			cumulus.LogMessage("GetRainFallTotals: This Month = " + Today);
			...
			cumulus.LogMessage("GetRainFallTotals: Rainthismonth from daily data: " + RainThisMonth);
			cumulus.LogMessage("GetRainFallTotals: Rainthisyear from daily data: " + RainThisYear);

			// Add in year-to-date rain (if necessary)
			if (cumulus.YTDrainyear == Convert.ToInt32(Today.Substring(6, 2)) + 2000)
			{
				cumulus.LogMessage("GetRainFallTotals: Adding YTD rain: " + cumulus.YTDrain);
				RainThisYear += cumulus.YTDrain;
				cumulus.LogMessage("GetRainFallTotals: Rainthisyear: " + RainThisYear);
			}
		}
		...
		public void ReadTodayFile()
		{
			...
			cumulus.LogMessage("ReadTodayFile: Read today file: Date = " + todayfiledate + ", LastUpdateTime = " + cumulus.LastUpdateTime + ", Month = " + CurrentMonth);
		...
		public void WriteTodayFile(DateTime timestamp, bool Log)
		{
			try
			{
				...
				if (Log)
				{
					cumulus.LogMessage($"Writing today.ini, LastUpdateTime = {cumulus.LastUpdateTime} raindaystart = {RainCounterDayStart:F2} rain counter = {RainCounter:F2}");
		...
		*/
		...
		public double GetWindRunMonth(int year, int month)
		{
			...
			return Database.ExecuteScalar<double>("SELECT SUM(WindRun) FROM Dayfile WHERE Date >= @0 AND Date < @1", startDate, enddate);
		}
		...
		*/
		...
		internal async Task sendWebSocketData(bool wait = false)
		{
			...
			try
			{
				...
				// ** CMX 4 - the broadacst is now awaitable, so we can run it synchronously - therefore now no need to add an artificial delay
				//Thread.Sleep(500);
			}
			catch (Exception ex)
			{
		...
		private static string getTimeString(TimeSpan timespan, string format = "HH:mm")
		{
			try
			{
		...
		private void RemoveOldRecentData(DateTime ts)
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "RemoveOldRecentData: Failed to delete old data");
			}
		}
		...
		private void MinuteChanged(DateTime now)
		{
			...
			if (!DataStopped)
			{
				...
				*/
				...
				if (cumulus.MySqlFunction.Settings.CustomMins.Enabled && now.Minute % cumulus.MySqlFunction.Settings.CustomMins.Interval == 0)
				{
					_ = cumulus.MySqlFunction.CustomMinutesTimerTick();  // let this run in background
				}
				...
				if (cumulus.MySqlFunction.Settings.CustomTimed.Enabled)
				{
					cumulus.MySqlFunction.CustomMySqlTimedUpdate(now);
				}
		...
		private void MinutePlus12Changed(DateTime now)
		{
			...
			try
			{
				if (!System.Runtime.InteropServices.RuntimeInformation.IsOSPlatform(System.Runtime.InteropServices.OSPlatform.Windows) && File.Exists("/sys/class/thermal/thermal_zone0/temp"))
				{
		...
		*/
		...
		public string CreateWxnowFileString()
		{
			...
			string data = String.Format("{0}\n{1:000}/{2:000}g{3:000}t{4}r{5:000}p{6:000}P{7:000}h{8:00}b{9:00000}",
				timestamp, AvgBearing, mphwind, mphgust, ftempstr, in100rainlasthour, in100rainlast24hours, in100raintoday, hum, mb10press);
			...
			if (!String.IsNullOrWhiteSpace(cumulus.WxnowComment))
			{
				var tokenParser = new TokenParser(cumulus.TokenParserOnToken)
				{
					...
					InputText = cumulus.WxnowComment
				};
		...
		*/
		...
		public void DoHumidity(int? humpar, DateTime timestamp)
		{
			...
			if ((previousHum != 999) && (Math.Abs(humpar.Value - previousHum) > cumulus.Spike.HumidityDiff))
			{
				...
				lastSpikeRemoval = DateTime.Now;
				cumulus.SpikeAlarm.LastMessage = $"Humidity difference greater than spike value - NewVal={humpar.Value} OldVal={previousHum}";
				cumulus.SpikeAlarm.Triggered = true;
				return;
			}
			...
			Humidity = (int?) cumulus.Calib.Hum.Calibrate(Humidity);
		...
		public void DoTemperature(double? temp, DateTime timestamp)
		{
			...
			// UpdateStatusPanel;
			// update global temp
			Temperature = cumulus.Calib.Temp.Calibrate(temp);
		...
		public void DoRain(double? total, double? rate, DateTime timestamp)
		{
...
#if DEBUG
			cumulus.LogDebugMessage($"DoRain: counter={total:f3}, rate={rate:f3}; RainToday={RainToday:f3}, StartOfDay={RainCounterDayStart:f3}");
#endif
			...
			*/
			...
			if (Math.Round(RainCounterDayStart, cumulus.RainDPlaces) - Math.Round(RainCounter, cumulus.RainDPlaces) > 0)
			{
				...
				{
					cumulus.LogMessage(" ****Rain counter reset confirmed: raindaystart = " + RainCounterDayStart + ", Raincounter = " + RainCounter);

					// set the start of day figure so it reflects the rain
					// so far today
					RainCounterDayStart = RainCounter - (RainToday.Value / cumulus.Calib.Rain.Mult);
			...
			{
				// Has a tip occurred?
				if (Math.Round(total.Value, cumulus.RainDPlaces) - Math.Round(previoustotal, cumulus.RainDPlaces) > 0)
				{
					// rain has occurred
		...
		public void DoWind(double? gustpar, int? bearingpar, double? speedpar, DateTime timestamp)
		{
			...
			var uncalibratedspeed = speedpar ?? WindAverageUncalibrated;
			var calibratedspeed = cumulus.Calib.WindSpeed.Calibrate(speedpar);


			WindLatest = cumulus.StationOptions.UseSpeedForLatest ? calibratedspeed : calibratedgust;

			windspeeds[nextwindvalue] = calibratedgust ?? 0;
			...
			{
				ThisMonth.HighGust.Val = calibratedgust.Value;
				ThisMonth.HighGust.Ts = timestamp;
				WriteMonthIniFile();
			}
			if (calibratedgust > ThisYear.HighGust.Val)
			{
				ThisYear.HighGust.Val = calibratedgust.Value;
			...
			{
				SetAlltime(AllTime.HighGust, calibratedgust.Value, timestamp);
			}

			// check for records (and set)
			CheckHighGust(calibratedgust, Bearing, timestamp);

			RecentWind[nextwind].Gust = gustpar.Value; // We store uncalibrated gust values, so if we need to calculate the average from them we do not need to uncalibrate
			RecentWind[nextwind].Speed = uncalibratedspeed ?? 0;
			RecentWind[nextwind].Timestamp = timestamp;
			...
			if (cumulus.StationOptions.CalcuateAverageWindSpeed)
			{
				int count = 0;
				double totalwind = 0;
				var fromAvgTime = timestamp - cumulus.AvgSpeedTime;

				for (int i = 0; i < MaxWindRecent; i++)
				{
					if (RecentWind[i].Timestamp >= fromAvgTime)
					{
#if DEBUGWIND
//						cumulus.LogDebugMessage($"Wind Time:{WindRecent[i].Timestamp.ToLongTimeString()} Gust:{WindRecent[i].Gust:F1} Speed:{WindRecent[i].Speed:F1}");
#endif
						count++;
						totalwind += cumulus.StationOptions.UseSpeedForAvgCalc ? RecentWind[i].Speed : RecentWind[i].Gust;
					}
				}

				// average the values
				WindAverageUncalibrated = count > 0 ? totalwind / count : null;
			...
			if (CalcRecentMaxGust)
			{
				...
				double maxgust = 0;
				int count = 0;
				var fromMaxTime = timestamp - cumulus.PeakGustTime;

				for (int i = 0; i <= MaxWindRecent - 1; i++)
				{
					if (RecentWind[i].Timestamp >= fromMaxTime && RecentWind[i].Gust > maxgust)
					{
						count++;
						maxgust = RecentWind[i].Gust;
					}
				}
				RecentMaxGust = count > 0 ? cumulus.Calib.WindGust.Calibrate(maxgust) : null;
			}
			else
			{
			...
			if (calibratedgust.HasValue)
			{
				WindVec[nextwindvec].X = calibratedgust.Value * Math.Sin(Trig.DegToRad(Bearing.Value));
				WindVec[nextwindvec].Y = calibratedgust.Value * Math.Cos(Trig.DegToRad(Bearing.Value));
			...
			double totalwindY = 0;
			int numvalues = 0;
			var fromBrgTime = timestamp - cumulus.AvgBearingTime;

			for (int i = 0; i < MaxWindRecent; i++)
			{
				if (WindVec[i].Timestamp >= fromBrgTime)
				{
					numvalues++;
		...
		public void DoUV(double? value, DateTime timestamp)
		{
			if (value.HasValue)
			{
				SolarRad = (int) cumulus.Calib.Solar.Calibrate(value);
		...
		public void InitialiseWind()
		{
			...
			for (int i = 0; i < MaxWindRecent; i++)
			{
				if (RecentWind[i].Timestamp >= fromTime)
				{
					numvalues++;
					totalwind += RecentWind[i].Speed;
				}
			}
			...
			for (int i = 0; i < MaxWindRecent; i++)
			{
				if (RecentWind[i].Timestamp >= fromTime)
				{
					if (RecentWind[i].Gust > RecentMaxGust)
					{
						RecentMaxGust = RecentWind[i].Gust;
					}
				}
			}
		...
		{
			var calGust = cumulus.Calib.WindGust.Calibrate(gust).Value;
			int calBearing;
			...
			{
				RecentWind[nextwind].Gust = gust;
				RecentWind[nextwind].Speed = speed;
				RecentWind[nextwind].Timestamp = ts;
				nextwind = (nextwind + 1) % MaxWindRecent;
		...
		public void DoSolarRad(int? value, DateTime timestamp)
		{
			...
			{
				try
				{ // catch any errors in the calibration
					SolarRad = (int)Math.Round(cumulus.Calib.Solar.Calibrate(value) ?? 0);
				}
				catch
				{
					SolarRad = 0;
		...
		public void DayReset(DateTime timestamp)
		{
			...
			if (drday != DayResetDay)
			{
				...
				{
					_ = cumulus.MySqlFunction.CustomRolloverTimerTick();
				}
				...
				if (day == 1)
				{
					...
					ThisMonth.HighGust.Val = calibratedgust ?? Cumulus.DefaultHiVal;
				...
				if ((day == 1) && (month == 1))
				{
					...
					ThisYear.HighGust.Val = calibratedgust ?? Cumulus.DefaultHiVal;
				...
				Graphs.CreateDailyGraphDataFiles();
		...
		private async Task DoDayfile(DateTime timestamp)
		{
			...
			{
				cumulus.MySqlFunction.DoDailyData(timestamp, AvgTemp);
			}
		}
			...
			double? appTemp, double? insideTemp, int? insideHum, int? solarMax, double? rainrate)
		{
			...
			switch ((Cumulus.PrimaryAqSensor) cumulus.StationOptions.PrimaryAqSensor)
			{
				case Cumulus.PrimaryAqSensor.AirLinkOutdoor:
					...
					break;
				case  Cumulus.PrimaryAqSensor.AirLinkIndoor:
					...
					break;
				case Cumulus.PrimaryAqSensor.Ecowitt1:
					pm2p5 = AirQuality[1];
					break;
				case Cumulus.PrimaryAqSensor.Ecowitt2:
					pm2p5 = AirQuality[2];
					break;
				case Cumulus.PrimaryAqSensor.Ecowitt3:
					pm2p5 = AirQuality[3];
					break;
				case Cumulus.PrimaryAqSensor.Ecowitt4:
					pm2p5 = AirQuality[4];
					break;
				case Cumulus.PrimaryAqSensor.EcowittCO2:
					pm2p5 = CO2_pm2p5;
					pm10 = CO2_pm10;
					break;
		...
		*/
		...
		private void LoadLast3HourData(DateTime ts)
		{
			...
			foreach (var rec in result)
			{
				try
				{
					...
					{
						RecentWind[nextwind].Gust = rec.WindGust.Value;
						RecentWind[nextwind].Speed = rec.WindSpeed.Value;
						RecentWind[nextwind].Timestamp = rec.Time;
						nextwind = (nextwind + 1) % MaxWindRecent;
					}
		...
		private void LoadWindData()
		{
			...
			try
			{
				...
				var result =  Database.Table<WindRecent>();

				cnt = result.Count();

				foreach (var rec in result)
				{
					RecentWind[nextwindvalue].Gust = rec.Gust;
					RecentWind[nextwindvalue].Speed = rec.Speed;
					RecentWind[nextwindvalue].Timestamp = rec.Time;
					nextwindvalue = (nextwindvalue + 1) % MaxWindRecent;
					if (numwindvalues < maxwindvalues)
					{
						numwindvalues++;
			...
			try
			{
				nextwind = Database.ExecuteScalar<int>("select * from WindRecentPointer limit 1");
			}
			catch (Exception ex)
			{
				cumulus.LogExceptionMessage(ex, "LoadWindData: Error loading pointer from database");
			}

			cumulus.LogMessage($"LoadWindData: Loaded {cnt} entries to WindRecent data list");
		}
		...
		public void SaveWindData()
		{
			...
			try
			{
				...
				Database.DeleteAll<WindRecent>();
				Database.Execute("delete from WindRecentPointer");

				// save the type array
				for (int i = 0; i < numwindvalues; i++)
				{
					Database.InsertOrReplace(new WindRecent()
					{
						Gust = RecentWind[nextwindvalue].Gust,
						Speed = RecentWind[nextwindvalue].Speed,
						Time = RecentWind[nextwindvalue].Timestamp
					});
					nextwindvalue = (nextwindvalue + 1) % MaxWindRecent;
				}

				// and save the pointer
				Database.Execute("insert into WindRecentPointer (pntr) values (?)", nextwind);
			...
			{
				cumulus.LogExceptionMessage(ex, "SaveWindData: Error saving RecentWind to the database");
				Database.Rollback();
			}
		}
		...
		*/
		...
		public string GetExtraTemp()
		{
			...
			for (int i = 1; i < 11; i++)
			{
				if (cumulus.GraphOptions.Visible.ExtraTemp.ValVisible(i - 1, true))
				{
					json.Append($"[\"{cumulus.Trans.ExtraTempCaptions[i - 1]}\",\"{(ExtraTemp[i].HasValue ? ExtraTemp[i].Value : "-"):cumulus.TempFormat}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
				}
			}
		...
		public string GetUserTemp()
		{
			...
			for (int i = 1; i < 9; i++)
			{
				if (cumulus.GraphOptions.Visible.UserTemp.ValVisible(i - 1, true))
				{
					json.Append($"[\"{cumulus.Trans.UserTempCaptions[i - 1]}\",\"{(UserTemp[i].HasValue ? UserTemp[i].Value : "-"):cumulus.TempFormat}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
				}
			}
		...
		public string GetExtraHum()
		{
			...
			for (int i = 1; i < 11; i++)
			{
				if (cumulus.GraphOptions.Visible.ExtraHum.ValVisible(i - 1, true))
				{
					json.Append($"[\"{cumulus.Trans.ExtraHumCaptions[i - 1]}\",\"{(ExtraHum[i].HasValue ? ExtraHum[i].Value : "-")}\",\"%\"],");
				}
			}
		...
		public string GetExtraDew()
		{
			...
			for (int i = 1; i < 11; i++)
			{
				if (cumulus.GraphOptions.Visible.ExtraDewPoint.ValVisible(i - 1, true))
				{
					json.Append($"[\"{cumulus.Trans.ExtraDPCaptions[i - 1]}\",\"{(ExtraDewPoint[i].HasValue ? ExtraDewPoint[i].Value : "-"):cumulus.TempFormat}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
				}
			}
		...
		public string GetSoilTemp()
		{
			...
			for (int i = 1; i <= 16; i++)
			{
				...
				{
					json.Append($"[\"{cumulus.Trans.SoilTempCaptions[i - 1]}\",\"{(SoilTemp[i].HasValue ? SoilTemp[i].Value : "-"):cumulus.TempFormat}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
				}
			}
		...
		public string GetSoilMoisture()
		{
			...
			for (var i = 1; i < SoilMoisture.Length; i++)
			{
				...
				{
					json.Append($"[\"{cumulus.Trans.SoilMoistureCaptions[i - 1]}\",\"{(SoilMoisture[i].HasValue ? SoilMoisture[i].Value : "-"):F0}\",\"{cumulus.Units.SoilMoistureUnitText}\"],");
				}
			}
			json.Length--;
			json.Append("]}");
		...
		public string GetAirQuality()
		{
			...
			json.Append($"[\"{cumulus.Trans.AirQualityCaptions[1]}\",\"{(AirQuality[1].HasValue ? AirQuality[1].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityCaptions[2]}\",\"{(AirQuality[2].HasValue ? AirQuality[2].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityCaptions[3]}\",\"{(AirQuality[3].HasValue ? AirQuality[3].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityCaptions[4]}\",\"{(AirQuality[4].HasValue ? AirQuality[4].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityAvgCaptions[1]}\",\"{(AirQualityAvg[1].HasValue ? AirQualityAvg[1].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityAvgCaptions[2]}\",\"{(AirQualityAvg[2].HasValue ? AirQualityAvg[2].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityAvgCaptions[3]}\",\"{(AirQualityAvg[3].HasValue ? AirQualityAvg[3].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.AirQualityAvgCaptions[4]}\",\"{(AirQualityAvg[4].HasValue ? AirQualityAvg[4].Value : "-"):F1}\",\"{cumulus.Units.AirQualityUnitText}\"]");
			json.Append("]}");
			return json.ToString();
		}
		...
		public string GetLeaf()
		{
			...
			//json.Append($"[\"{cumulus.Trans.LeafTempCaptions[1]}\",\"{(LeafTemp[1].HasValue ? LeafTemp[1].Value.ToString(cumulus.TempFormat) : "-")}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
			//json.Append($"[\"{cumulus.Trans.LeafTempCaptions[2]}\",\"{(LeafTemp[2].HasValue ? LeafTemp[2].Value.ToString(cumulus.TempFormat) : "-")}\",\"&deg;{cumulus.Units.TempText[1]}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[1]}\",\"{(LeafWetness[1].HasValue ? LeafWetness[1].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[2]}\",\"{(LeafWetness[2].HasValue ? LeafWetness[2].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"]");
			json.Append("]}");
			return json.ToString();
		}
		...
		*/
		...
		public string GetLeaf8()
		{
			...
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[1]}\",\"{(LeafWetness[1].HasValue ? LeafWetness[1].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[2]}\",\"{(LeafWetness[2].HasValue ? LeafWetness[2].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[3]}\",\"{(LeafWetness[3].HasValue ? LeafWetness[3].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[4]}\",\"{(LeafWetness[4].HasValue ? LeafWetness[4].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[5]}\",\"{(LeafWetness[5].HasValue ? LeafWetness[5].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[6]}\",\"{(LeafWetness[6].HasValue ? LeafWetness[6].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[7]}\",\"{(LeafWetness[7].HasValue ? LeafWetness[7].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"],");
			json.Append($"[\"{cumulus.Trans.LeafWetnessCaptions[8]}\",\"{(LeafWetness[8].HasValue ? LeafWetness[8].Value.ToString(cumulus.LeafWetFormat) : "-")}\",\"{cumulus.Units.LeafWetnessUnitText}\"]");
			json.Append("]}");
			return json.ToString();
		}
		...
		*/
		...
		public string GetTodayYestTemp()
		{
			...
			json.Append(HiLoToday.HighTemp.HasValue ? HiLoToday.HighTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighTemp.HasValue ? HiLoYest.HighTemp.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.HighTemp.HasValue ? HiLoYest.HighTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.LowTemp.HasValue ? HiLoToday.LowTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowTemp.HasValue ? HiLoYest.LowTemp.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.LowTemp.HasValue ? HiLoYest.LowTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.HighAppTemp.HasValue ? HiLoToday.HighAppTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighAppTemp.HasValue ? HiLoYest.HighAppTemp.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.HighAppTemp.HasValue ? HiLoYest.HighAppTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.LowAppTemp.HasValue ? HiLoToday.LowAppTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowAppTemp.HasValue ? HiLoYest.LowAppTemp.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.LowAppTemp.HasValue ? HiLoYest.LowAppTempTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.HighFeelsLike.HasValue ? HiLoToday.HighFeelsLikeTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighFeelsLike.HasValue ? HiLoYest.HighFeelsLike.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.HighFeelsLike.HasValue ? HiLoYest.HighFeelsLikeTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.LowFeelsLike.HasValue ? HiLoToday.LowFeelsLikeTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowFeelsLike.HasValue ? HiLoYest.LowFeelsLike.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.LowFeelsLike.HasValue ? HiLoYest.LowFeelsLikeTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append("\",\"");
			json.Append(HiLoToday.HighHumidex.HasValue ? HiLoToday.HighHumidexTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighHumidex.HasValue ? HiLoYest.HighHumidex.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append("\",\"");
			json.Append(HiLoYest.HighHumidex.HasValue ? HiLoYest.HighHumidexTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.HighDewPoint.HasValue ? HiLoToday.HighDewPointTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighDewPoint.HasValue ? HiLoYest.HighDewPoint.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.HighDewPoint.HasValue ? HiLoYest.HighDewPointTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.LowDewPoint.HasValue ? HiLoToday.LowDewPointTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowDewPoint.HasValue ? HiLoYest.LowDewPoint.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.LowDewPoint.HasValue ? HiLoYest.LowDewPointTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.LowWindChill.HasValue ? HiLoToday.LowWindChillTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowWindChill.HasValue ? HiLoYest.LowWindChill.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.LowWindChill.HasValue ? HiLoYest.LowWindChillTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(closeStr);
			...
			json.Append(tempUnitStr);
			json.Append(HiLoToday.HighHeatIndex.HasValue ? HiLoToday.HighHeatIndexTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighHeatIndex.HasValue ? HiLoYest.HighHeatIndex.Value.ToString(cumulus.TempFormat) : nullVal);
			json.Append(tempUnitStr);
			json.Append(HiLoYest.HighHeatIndex.HasValue ? HiLoYest.HighHeatIndexTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"]");
		...
		public string GetTodayYestHum()
		{
			...
			json.Append(unitStr);
			json.Append(HiLoToday.HighHumidity.HasValue ? HiLoToday.HighHumidityTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighHumidity.HasValue ? HiLoYest.HighHumidity.Value.ToString(cumulus.HumFormat) : nullVal);
			json.Append(unitStr);
			json.Append(HiLoYest.HighHumidity.HasValue ? HiLoYest.HighHumidityTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
			...
			json.Append(unitStr);
			json.Append(HiLoToday.LowHumidity.HasValue ? HiLoToday.LowHumidityTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowHumidity.HasValue ? HiLoYest.LowHumidity.Value.ToString(cumulus.HumFormat) : nullVal);
			json.Append(unitStr);
			json.Append(HiLoYest.LowHumidity.HasValue ? HiLoYest.LowHumidityTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"]");
		...
		public string GetTodayYestRain()
		{
			...
			json.Append(sepStr);
			json.Append(HiLoToday.HighRainRate.HasValue ? HiLoToday.HighRainRateTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighRainRate.HasValue ? HiLoYest.HighRainRate.Value.ToString(cumulus.RainFormat) : nullVal);
			json.Append(unitStr + "/hr");
			json.Append(sepStr);
			json.Append(HiLoYest.HighRainRate.HasValue ? HiLoYest.HighRainRateTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
		...
		public string GetTodayYestWind()
		{
			...
			json.Append(sepStr);
			json.Append(HiLoToday.HighGust.HasValue ? HiLoToday.HighGustTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighGust.HasValue ? HiLoYest.HighGust.Value.ToString(cumulus.WindFormat) : nullVal);
			json.Append("&nbsp;" + cumulus.Units.WindText);
			json.Append(sepStr);
			json.Append(HiLoYest.HighGust.HasValue ? HiLoYest.HighGustTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
			...
			json.Append("&nbsp;" + cumulus.Units.WindText);
			json.Append(sepStr);
			json.Append(HiLoToday.HighWind.HasValue ? HiLoToday.HighWindTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighWind.HasValue ? HiLoYest.HighWind.Value.ToString(cumulus.WindAvgFormat) : nullVal);
			json.Append("&nbsp;" + cumulus.Units.WindText);
			json.Append(sepStr);
			json.Append(HiLoYest.HighWind.HasValue ? HiLoYest.HighWindTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
		...
		public string GetTodayYestPressure()
		{
			...
			json.Append(unitStr);
			json.Append(sepStr);
			json.Append(HiLoToday.HighPress.HasValue ? HiLoToday.HighPressTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighPress.HasValue ? HiLoYest.HighPress.Value.ToString(cumulus.PressFormat) : nullVal);
			json.Append(unitStr);
			json.Append(sepStr);
			json.Append(HiLoYest.HighPress.HasValue ? HiLoYest.HighPressTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
			...
			json.Append(unitStr);
			json.Append(sepStr);
			json.Append(HiLoToday.LowPress.HasValue ? HiLoToday.LowPressTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.LowPress.HasValue ? HiLoYest.LowPress.Value.ToString(cumulus.PressFormat) : nullVal);
			json.Append(unitStr);
			json.Append(sepStr);
			json.Append(HiLoYest.LowPress.HasValue ? HiLoYest.LowPressTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"]");
		...
		public string GetTodayYestSolar()
		{
			...
			json.Append("&nbsp;W/m2");
			json.Append(sepStr);
			json.Append(HiLoToday.HighSolar.HasValue ? HiLoToday.HighSolarTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighSolar.HasValue ? HiLoYest.HighSolar.Value : nullVal);
			json.Append("&nbsp;W/m2");
			json.Append(sepStr);
			json.Append(HiLoYest.HighSolar.HasValue ? HiLoYest.HighSolarTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"],");
			...
			json.Append(HiLoToday.HighUv.HasValue ? HiLoToday.HighUv.Value.ToString("F1") : nullVal);
			json.Append("&nbsp;");
			json.Append(sepStr);
			json.Append(HiLoToday.HighUv.HasValue ? HiLoToday.HighUvTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append(sepStr);
			json.Append(HiLoYest.HighUv.HasValue ? HiLoYest.HighUv.Value.ToString("F1") : nullVal);
			json.Append("&nbsp;");
			json.Append(sepStr);
			json.Append(HiLoYest.HighUv.HasValue ? HiLoYest.HighUvTime.ToString(cumulus.ProgramOptions.TimeFormat) : nullTime);
			json.Append("\"]");
		...
		public static string GetCachedSqlCommands(string draw, int start, int length, string search)
		{
			try
			{
				...
				var json = new StringBuilder(350 * cumulus.MySqlFunction.FailedList.Count);

				json.Append("{\"data\":[");

				//var lines = File.ReadLines(cumulus.DayFile).Skip(start).Take(length);

				foreach (var rec in cumulus.MySqlFunction.FailedList)
				{
				...
				json.Append(cumulus.MySqlFunction.FailedList.Count);
				json.Append(",\"draw\":");
				json.Append(draw);
				json.Append(",\"recordsFiltered\":");
				json.Append(string.IsNullOrEmpty(search) ? cumulus.MySqlFunction.FailedList.Count : filtered);
				json.Append('}');
		...
		internal string GetCurrentData()
		{
			...
			var data = new WebSocketData(cumulus, Temperature, Humidity, TempTotalToday / tempsamplestoday, IndoorTemp, Dewpoint, WindChill, IndoorHum,
				...
				RainLastHour, HeatIndex, Humidex, ApparentTemp, temptrendval, presstrendval, HiLoToday.HighGust, HiLoToday.HighGustTime.ToString(cumulus.ProgramOptions.TimeFormat), HiLoToday.HighWind,
				...
				HiLoToday.HighUvTime.ToString(cumulus.ProgramOptions.TimeFormat), forecaststr, getTimeString(cumulus.SunRiseTime), getTimeString(cumulus.SunSetTime),
		...
		public bool CheckHighGust(double? gust, int? gustdir, DateTime timestamp)
		{
			...
			if (gust.Value > HiLoToday.HighGust)
			{
				HiLoToday.HighGust = gust.Value;
				HiLoToday.HighGustTime = timestamp;
				HiLoToday.HighGustBearing = gustdir;
				WriteTodayFile(timestamp, false);
			}
			if (gust.Value > ThisMonth.HighGust.Val)
			{
				ThisMonth.HighGust.Val = gust.Value;
				ThisMonth.HighGust.Ts = timestamp;
				WriteMonthIniFile();
			}
			if (gust.Value > ThisYear.HighGust.Val)
			{
				ThisYear.HighGust.Val = gust.Value;
				ThisYear.HighGust.Ts = timestamp;
				WriteYearIniFile();
			}
			// All time high gust?
			if (gust.Value > AllTime.HighGust.Val)
			{
				SetAlltime(AllTime.HighGust, gust.Value, timestamp);
			}

			// check for monthly all time records (and set)
			CheckMonthlyAlltime("HighGust", gust.Value, true, timestamp);

			cumulus.HighGustAlarm.CheckAlarm(gust.Value);
		...
		public void CheckHighAvgSpeed(DateTime timestamp)
		{
			...
			cumulus.HighWindAlarm.CheckAlarm(WindAverage.Value);
		}
