		...
		private void EWGetData(object sender, ElapsedEventArgs elapsedEventArgs)
		{
			if (File.Exists(cumulus.EwOptions.Filename))
			{
				try
				{
					...
					DoTemperature(ConvertUnits.TempCToUser(GetConvertedValue(st[EW_OUTDOOR_TEMP])), now);
					...
					DoDewpoint(ConvertUnits.TempCToUser(GetConvertedValue(st[EW_DEW_POINT])), now);
