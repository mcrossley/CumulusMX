...
namespace CumulusMX.Stations
{
	internal class EcowittCloudStation : WeatherStation
	{
		...
		public EcowittCloudStation(Cumulus cumulus, WeatherStation station = null) : base(cumulus)
		{
			...
			if (main || (!main && cumulus.EcowittSettings.ExtraUseAQI))
			{
				cumulus.Units.AirQualityUnitText = "µg/m³";
			}
			if (main || (!main && cumulus.EcowittSettings.ExtraUseSoilMoist))
			{
			...
			else if (cumulus.EcowittSettings.ExtraUseCamera)
			{
		...
		{
			if ((cumulus.EcowittSettings.ExtraUseCamera || main) && cumulus.EcowittSettings.CameraMacAddress != null)
			{
				try
				{
		...
		private void GetHistoricData()
	{
		...
		private void ProcessCurrentData(EcowittApi.CurrentDataData data, CancellationToken token)
		{
			...
			if (main)
			{
				...
				if (cumulus.Gw1000PrimaryTHSensor == 0)
				{
					...
					else
					{
						try
						{
							...
							DoTemperature(data.outdoor.temperature.value, time);
							DoHumidity(data.outdoor.humidity.value, time);
							DoDewpoint(data.outdoor.dew_point.value, time);
							DoFeelsLike(time);
							DoApparentTemp(time);
							DoHumidex(time);
				...
				else
				{
					try
					{
						...
						{
							var time = Utils.FromUnixTime(data.outdoor.temperature.time);
							DoTemperature(data.indoor.temperature.value, time);
							DoHumidity(data.indoor.humidity.value, time);
							DoDewpoint(data.outdoor.dew_point.value, time);
							DoFeelsLike(time);
							DoApparentTemp(time);
							DoHumidex(time);
			...
			// Solar
			if ((main || cumulus.EcowittSettings.ExtraUseSolar) && data.solar_and_uvi != null)
			{
				try
				{
		...
		private void ProcessExtraTempHum(EcowittApi.CurrentDataData data, WeatherStation station)
		{
			if (data.temp_and_humidity_ch1 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch1.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch1.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch1.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch1.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch2 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch2.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch2.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch2.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch2.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch3 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch3.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch3.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch3.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch3.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch4 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch4.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch4.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch4.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch4.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch5 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch5.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch5.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch5.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch5.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch6 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch6.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch6.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch6.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch6.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch7 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch7.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch7.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch7.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch7.humidity.time));
				}
			...
			if (data.temp_and_humidity_ch8 != null)
			{
				...
				{
					station.DoTemperature(data.temp_and_humidity_ch8.temperature.value, Utils.FromUnixTime(data.temp_and_humidity_ch8.temperature.time));
					station.DoHumidity(data.temp_and_humidity_ch8.humidity.value, Utils.FromUnixTime(data.temp_and_humidity_ch8.humidity.time));
				}
		...
		private void ProcessUserTemps(EcowittApi.CurrentDataData data, WeatherStation station)
		{
			if (data.temp_ch1 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch1.temperature.value, cumulus.EcowittSettings.MapWN34[1]);
				}
			}
			...
			if (data.temp_ch2 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch2.temperature.value, cumulus.EcowittSettings.MapWN34[2]);
				}
			}
			...
			if (data.temp_ch3 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch3.temperature.value, cumulus.EcowittSettings.MapWN34[3]);
				}
			}
			...
			if (data.temp_ch4 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch4.temperature.value, cumulus.EcowittSettings.MapWN34[4]);
				}
			}
			...
			if (data.temp_ch5 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch5.temperature.value, cumulus.EcowittSettings.MapWN34[5]);
				}
			}
			...
			if (data.temp_ch6 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch6.temperature.value, cumulus.EcowittSettings.MapWN34[6]);
				}
			}
			...
			if (data.temp_ch7 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch7.temperature.value, cumulus.EcowittSettings.MapWN34[7]);
				}
			}
			...
			if (data.temp_ch8 != null)
			{
				...
				{
					station.DoSoilTemp(data.temp_ch8.temperature.value, cumulus.EcowittSettings.MapWN34[8]);
				}
			}
		}
		...
		private void ProcessLightning(EcowittApi.CurrentDataData data, WeatherStation station)
		{
			if (data.lightning != null)
			{
				if (data.lightning.distance != null && data.lightning.distance.value != 255)
				{
					...
					station.LightningDistance = ConvertUnits.KmtoUserUnits(data.lightning.distance.value).Value;
