	...
	internal class DavisAirLink
	{
			...
			if (CheckIpValid(ip))
			{
				...
				do
				{
					...
					try
					{
						...
						{
							cumulus.LogExceptionMessage(ex, "GetAlCurrent: Error processing the AirLink response");
						}
						retry = 9;
					}
					catch (Exception ex)
					{
						...
						if (retry == 3)
						{
							...
							{
								cumulus.LogMessage("GetAlCurrent: Error - The AirLink rejected our request");
							}
							else if (ex is TimeoutException)
							{
								cumulus.LogMessage("GetAlCurrent: Error - Timed out waiting for AirLink response");
							}
							else
							{
		...
		private void DecodeAlCurrent(string currentJson)
		{
				...
				switch (type)
				{
					...
					case 6: // AirLink - newer firmware
						...
						try
						{
							...
							if (indoor)
							{
								cumulus.airLinkDataIn.temperature = ConvertUnits.TempFToUser(rec.temp).Value;
							}
							else
							{
								cumulus.airLinkDataOut.temperature = ConvertUnits.TempFToUser(rec.temp).Value;
							}
						}
						catch (Exception ex)
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlCurrent: {locationStr} - Error processing temperature value");
						}
						...
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlCurrent: {locationStr} - Error processing humidity value");
						}
						...
						*/
						...
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlCurrent: {locationStr} - Error processing PM values");
						}
			...
			catch (Exception ex)
			{
				...
				cumulus.LogExceptionMessage(ex, $"DecodeAlCurrent: {locationStr}");
			}
		}
		...
		private void AlReadHistory(object sender, DoWorkEventArgs e)
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "AirLink: Exception occurred reading archive data");
			}
		}
		...
		private void GetWlHistoricData()
		{
			...
			try
			{
				...
				if (responseCode != 200)
				{
					...
					cumulus.LogWarningMessage($"GetWlHistoricData: WeatherLink API Historic Error: {errObj.code}, {errObj.message}");
			...
			{
				cumulus.LogExceptionMessage(ex, "GetWlHistoricData:  Error");
				airLinkLastUpdateTime = Utils.FromUnixTime(endTime);
				return;
			}
		...
		public void DecodeAlHistoric(int dataType, string json)
		{
			try
			{
				switch (dataType)
				{
					case 17: // AirLink Archive record
						...
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlHistoric: {locationStr} - Error processing avg temperature");
						}
						...
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlHistoric: {locationStr} - Error processing humidity");
						}
						...
						{
							cumulus.LogExceptionMessage(ex, $"DecodeAlHistoric: {locationStr} - Error processing PM data.");
						}
			...
			{
				cumulus.LogExceptionMessage(ex, $"DecodeAlHistoric: {locationStr}, Error");
			}
		}
		...
		private void GetWlHistoricHealth()
		{
			...
			try
			{
				...
				else if (responseBody.StartsWith("{\"")) // basic sanity check
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "AirLinkHealth: Error");
					}
				}
				else // No idea what we got, dump it to the log
				{
					cumulus.LogErrorMessage("AirLinkHealth: Invalid historic message received");
					cumulus.LogMessage("AirLinkHealth: Received: " + responseBody);
					return;
				}
			}
			catch (Exception ex)
		...
		internal void DecodeWlApiHealth(WlHistorySensor sensor, bool startingup)
		{
			if (sensor.data.Count == 0)
			{
				...
				{
					cumulus.LogWarningMessage("AirLinkHealth: Did not find any health data for AirLink device");
				}
				return;
			}
			...
			if (sensor.data_structure_type == 18)
			{
				...
				try
				{
					...
					{
						cumulus.LogExceptionMessage(ex, $"AirLinkHealth: {locationStr} - Error processing firmware version");
					...
					{
						cumulus.LogExceptionMessage(ex, $"AirLinkHealth: {locationStr} - Error processing uptime");
					}
					...
					{
						cumulus.LogExceptionMessage(ex, $"AirLinkHealth: {locationStr} - Error processing link uptime");
					}
					...
					{
						cumulus.LogExceptionMessage(ex, $"AirLinkHealth: {locationStr} - Error processing xmt count");
					}
				}
				catch (Exception ex)
				{
					cumulus.LogExceptionMessage(ex, $"AirLinkHealth: {locationStr} - Exception caught in health data");
				}
			}
		}
		...
		private bool GetAvailableStationIds()
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "WeatherLink API error");
			}
			return false;
		}
		...
		private void GetAvailableSensors()
		{
			...
			var logUrl = sensorsUrl.ToString().Replace(cumulus.AirLinkApiKey, "<<API_KEY>>");
			...
			{
				cumulus.LogExceptionMessage(ex, "GetAvailableSensors: WeatherLink API error");
			}
		}
