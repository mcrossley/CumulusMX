	...
	internal class DavisWllStation : WeatherStation
	{
			...
			try
			{
				...
				broadcastTask = Task.Run(async () =>
				{
					...
					while (!cumulus.cancellationToken.IsCancellationRequested)
					{
						try
						{
							var broadcastTask = udpClient.ReceiveAsync(cumulus.cancellationToken);
							...
							await Task.WhenAny(broadcastTask.AsTask(), timeoutTask);

							// we get duplicate packets over IPv4 and IPv6, plus if the host has multiple interfaces to the local LAN
							if (broadcastTask.IsCompletedSuccessfully && !Utils.ByteArraysEqual(lastMessage, broadcastTask.Result.Buffer))
							{
			...
			do
			{
				...
				try
				{
					...
					if (CheckIpValid(ip))
					{
						...
						LogRawStationData(urlRealtime, true);
				...
				catch (Exception ex)
				{
					...
					if (ex.InnerException != null && ex.InnerException.Message.Contains("response ended prematurely"))
						cumulus.LogDebugMessage("GetWllRealtime: Error - The WLL rejected our Realtime request");
					else
						cumulus.LogExceptionMessage(ex, "GetWllRealtime: Error");
		...
		private async void GetWllCurrent(object source, ElapsedEventArgs e)
		{
			...
			if (CheckIpValid(ip))
			{
				...
				do
				{
					...
					{
						string responseBody;
						LogRawStationData(urlCurrent, true);
					...
					catch (Exception ex)
					{
						...
						if (retry == 3)
						{
							...
							if (ex.InnerException != null && ex.InnerException.Message.Contains("response ended prematurely"))
							{
								cumulus.LogErrorMessage("GetWllCurrent: Error - The WLL rejected our request");
							}
							else if (ex is TimeoutException)
							{
								cumulus.LogErrorMessage("GetWllCurrent: Error - Timed out waiting for WLL response");
							}
							else
							{
		...
		private void DecodeBroadcast(string broadcastJson, IPEndPoint from)
		{
			try
			{
				...
				LogRawStationData(broadcastJson, false);
					...
					foreach (var rec in json.conditions)
					{
						...
						if (cumulus.WllPrimaryWind == rec.txid)
						{
							try
							{
								...
								DoWind(ConvertUnits.WindMPHToUser(rec.wind_speed_last), windDir, -1, dateTime);
							}
							catch (Exception ex)
							{
								cumulus.LogExceptionMessage(ex, $"WLL broadcast: Error in wind speed found on TxId {rec.txid}");
							}
						}
						...
						{
							cumulus.LogExceptionMessage(ex, $"WLL broadcast: no valid rainfall found on TxId {rec.txid}");
						}
					}
				...
				else if (broadcastJson.StartsWith("STR_BCAST"))
				{
					...
					cumulus.LogWarningMessage($"WLL broadcast: Received spurious message from printer utility(?) at IP address {from.Address} starting with \"STR_BCAST\"");
			...
			{
				cumulus.LogExceptionMessage(ex, "DecodeBroadcast(): Exception");
				multicastsBad++;
		...
		private void DecodeCurrent(string currentJson)
		{
				...
				foreach (var rec in json.data.conditions)
				{
					...
					switch (type)
					{
						case 1: // ISS
							...
							if (cumulus.WllPrimaryTempHum == data1.txid)
							{
								...
								try
								{
									...
									DoHumidity((int?)data1.hum, dateTime);

									DoTemperature(ConvertUnits.TempFToUser(data1.temp), dateTime);

									DoDewpoint(ConvertUnits.TempFToUser(data1.dew_point), dateTime);

									newWindChill = ConvertUnits.TempFToUser(data1.wind_chill);

									if (data1.thsw_index.HasValue)
									{
										THSWIndex = ConvertUnits.TempFToUser(data1.thsw_index);
									}
								...
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing temperature values on TxId {data1.txid}");
								}
							}
							else
							{   // Check for Extra temperature/humidity settings
								for (var tempTxId = 1; tempTxId <= 8; tempTxId++)
								{
									...
									{
										cumulus.LogExceptionMessage(ex, $"WLL current: Error processing Extra temperature/humidity values on TxId {data1.txid}");
									}
								}
							}
							...
							if (cumulus.WllPrimaryWind == data1.txid)
							{
								...
								*/
								try
								{
									...
									{
										cumulus.LogDebugMessage("WLL current: Setting max gust from current 10 min value: " + gustCal.Value.ToString(cumulus.WindFormat) + " was: " + RecentMaxGust.Value.ToString(cumulus.WindFormat));

										// add to recent values so normal calculation includes this value
										RecentWind[nextwind].Gust = gust.Value; // use uncalibrated value
										RecentWind[nextwind].Speed = currentAvgWindSpd.Value;
										RecentWind[nextwind].Timestamp = dateTime;
								...
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing wind speeds on TxId {data1.txid}");
								}
							}
							...
							if (cumulus.WllPrimaryRain == data1.txid)
							{
								...
								*/
								...
								else
								{
									...
									{
										cumulus.LogExceptionMessage(ex, $"WLL current: Error processing rain storm values on TxId {data1.txid}");
									}
								}
							}
							...
							if (cumulus.WllPrimaryUV == data1.txid && data1.uv_index.HasValue)
							{
								...
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing UV value on TxId {data1.txid}");
								}
							}
							...
							if (cumulus.WllPrimarySolar == data1.txid && data1.solar_rad.HasValue)
							{
								...
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing Solar value on TxId {data1.txid}");
								}
							}
							break;
						...
						case 2: // Leaf/Soil Moisture
							...
							{
								cumulus.LogExceptionMessage(ex, $"WLL current: Error processing LeafWetness txid={data2.txid}, idx={idx}");
							}
							...
							if (cumulus.WllExtraSoilMoistureTx1 == data2.txid)
							{
								...
								try
								{
									DoSoilMoisture((int?)data2[idx], 1);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing soil moisture #{cumulus.WllExtraSoilMoistureIdx1} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilMoistureTx2 == data2.txid)
							{
								...
								try
								{
									DoSoilMoisture((int?)data2[idx], 2);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing soil moisture #{cumulus.WllExtraSoilMoistureIdx2} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilMoistureTx3 == data2.txid)
							{
								...
								try
								{
									DoSoilMoisture((int?)data2[idx], 3);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing soil moisture #{cumulus.WllExtraSoilMoistureIdx3} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilMoistureTx4 == data2.txid)
							{
								...
								try
								{
									DoSoilMoisture((int?)data2[idx], 4);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing soil moisture #{cumulus.WllExtraSoilMoistureIdx4} on TxId {data2.txid}");
								}
							}
							...
							if (cumulus.WllExtraSoilTempTx1 == data2.txid)
							{
								...
								try
								{
									DoSoilTemp(ConvertUnits.TempFToUser((double?)data2[idx]), 1);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing extra soil temp #{cumulus.WllExtraSoilTempIdx1} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilTempTx2 == data2.txid)
							{
								...
								try
								{
									DoSoilTemp(ConvertUnits.TempFToUser((double?)data2[idx]), 2);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing extra soil temp #{cumulus.WllExtraSoilTempIdx2} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilTempTx3 == data2.txid)
							{
								...
								try
								{
									DoSoilTemp(ConvertUnits.TempFToUser((double?)data2[idx]), 3);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing extra soil temp #{cumulus.WllExtraSoilTempIdx3} on TxId {data2.txid}");
								}
							}
							if (cumulus.WllExtraSoilTempTx4 == data2.txid)
							{
								...
								try
								{
									DoSoilTemp(ConvertUnits.TempFToUser((double?)data2[idx]), 4);
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing extra soil temp #{cumulus.WllExtraSoilTempIdx4} on TxId {data2.txid}");
								}
							}
						...
						case 3: // Barometer
							...
							try
							{
								...
								DoPressure(ConvertUnits.PressINHGToUser(data3.bar_sea_level), dateTime);
								// Altimeter from absolute
								StationPressure = ConvertUnits.PressINHGToUser(data3.bar_absolute);
							...
							{
								cumulus.LogExceptionMessage(ex, "WLL current: Error processing baro data");
							}
						...
						case 4: // WLL Temp/Humidity
							...
							try
							{
								DoIndoorTemp(ConvertUnits.TempFToUser(data4.temp_in));
							}
							catch (Exception ex)
							{
								cumulus.LogExceptionMessage(ex, "WLL current: Error processing indoor temp data");
							}
							...
							{
								cumulus.LogExceptionMessage(ex, "WLL current: Error processing indoor humidity data");
							}
			...
			{
				cumulus.LogExceptionMessage(ex, "DecodeCurrent: Exception");
			}
		}
		...
		private static double ConvertRainClicksToUser(int clicks, int size)
		{
			// 0: Reserved, 1: 0.01", 2: 0.2mm, 3: 0.1mm, 4: 0.001"
			return size switch
			{
				1 => ConvertUnits.RainINToUser(clicks * 0.01).Value,
				2 => ConvertUnits.RainMMToUser(clicks * 0.2).Value,
				3 => ConvertUnits.RainMMToUser(clicks * 0.1).Value,
				4 => ConvertUnits.RainINToUser(clicks * 0.001).Value,
				_ => cumulus.DavisOptions.RainGaugeType switch
				{
					...
					0 => ConvertUnits.RainMMToUser(clicks * 0.2).Value,
					_ => ConvertUnits.RainINToUser(clicks * 0.01).Value,
				},
			};
		}
		...
		*/
		...
		private void GetWlHistoricData(BackgroundWorker worker)
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "GetWlHistoricData:  Exception:");
				cumulus.LastUpdateTime = Utils.FromUnixTime(endTime);
				return;
			}
		...
		private void DecodeHistoric(int dataType, int sensorType, string json)
		{
					...
					case 11: // ISS data
						...
						if (cumulus.WllPrimaryTempHum == data11.tx_id)
						{
							...
							try
							{
								...
								if (data11.hum_hi_at != 0 && data11.hum_hi != null)
								{
									...
									DoHumidity(Convert.ToInt32(data11.hum_hi), ts);
								}
								else
								{
								...
								if (data11.hum_lo_at != 0 && data11.hum_lo != null)
								{
									...
									DoHumidity(Convert.ToInt32(data11.hum_lo), ts);
								}
								else
								{
								...
								{
									// do current humidity
									DoHumidity(Convert.ToInt32(data11.hum_last), recordTs);
								}
								else
								{
									Humidity = null;
							...
							try
							{
								...
								else
								{
									...
									if (data11.temp_hi_at != 0 && data11.temp_hi != null)
									{
										...
										DoTemperature(ConvertUnits.TempFToUser(data11.temp_hi), ts);
									}
									else
									{
									...
									if (data11.temp_lo_at != 0 && data11.temp_lo != null)
									{
										...
										DoTemperature(ConvertUnits.TempFToUser(data11.temp_lo), ts);
									}
									else
									{
									...
									// do last temp
									DoTemperature(ConvertUnits.TempFToUser(data11.temp_last), recordTs);
									if (Temperature == null)
									{
									...
									// update chill hours
									if ((Temperature ?? Cumulus.DefaultLoVal) < cumulus.ChillHourThreshold)
									{
							...
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing Primary temperature value on TxId {data11.tx_id}");
							}
							...
							try
							{
								...
								if (data11.dew_point_hi_at != 0 && data11.dew_point_hi != null)
								{
									...
									DoDewpoint(ConvertUnits.TempFToUser((double)data11.dew_point_hi), ts);
								}
								else
								{
								...
								if (data11.dew_point_lo_at != 0 && data11.dew_point_lo != null)
								{
									...
									DoDewpoint(ConvertUnits.TempFToUser((double)data11.dew_point_lo), ts);
								}
								else
								{
								...
								{
									DoDewpoint(ConvertUnits.TempFToUser((double)data11.dew_point_last), recordTs);
								}
								else
								{
							...
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing dew point value on TxId {data11.tx_id}");
							}
							...
							if (!cumulus.StationOptions.CalculatedWC)
							{
								...
								try
								{
									...
									if (data11.wind_chill_lo_at != 0 && data11.wind_chill_lo != null)
									{
										...
										DoWindChill(ConvertUnits.TempFToUser(data11.wind_chill_lo), ts);
									}
									else
									{
									...
									{
										DoWindChill(ConvertUnits.TempFToUser(data11.wind_chill_last), recordTs);
									}
									else
									{
								...
								{
									cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing wind chill value on TxId {data11.tx_id}");
								}
							}
						}
						else
						{   // Check for Extra temperature/humidity settings
							for (var tempTxId = 1; tempTxId <= 8; tempTxId++)
							{
								...
								{
									cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing extra temp value on TxId {data11.tx_id}");
								}
								...
								{
									cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing extra humidity value on TxId {data11.tx_id}");
								}
							}
						}
							...
							*/
							...
							try
							{
								if (data11.wind_speed_hi != null && data11.wind_speed_hi_dir != null && data11.wind_speed_avg != null)
								{
									...
									DoWind(ConvertUnits.WindMPHToUser(data11.wind_speed_hi), data11.wind_speed_hi_dir, ConvertUnits.WindMPHToUser(data11.wind_speed_avg), recordTs);
								}
								else
								{
								...
								{
									WindAverage = cumulus.Calib.WindSpeed.Calibrate(ConvertUnits.WindMPHToUser(data11.wind_speed_avg));
							...
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing wind values on TxId {data11.tx_id}");
							}
						}
						...
						if (cumulus.WllPrimaryRain == data11.tx_id)
						{
							...
							try
							{
								if (data11.rain_rate_hi_at != 0 && data11.rainfall_clicks != null && data11.rain_rate_hi_clicks != null)
								{
									...
									var rain = ConvertRainClicksToUser(data11.rainfall_clicks.Value, data11.rain_size);
									var rainrate = ConvertRainClicksToUser(data11.rain_rate_hi_clicks.Value, data11.rain_size);
									if (rain > 0)
									{
							...
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing rain data on TxId {data11.tx_id}");
							}
						...
						if (cumulus.WllPrimaryUV == data11.tx_id)
						{
							...
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing UV value on TxId {data11.tx_id}");
							}
						...
						if (cumulus.WllPrimarySolar == data11.tx_id)
						{
							...
							try
							{
								...
								if (data11.et != null && !cumulus.StationOptions.CalculatedET)
								{
									...
									var newET = AnnualETTotal + ConvertUnits.RainINToUser(data11.et.Value).Value;
									cumulus.LogDebugMessage($"WLL DecodeHistoric: Adding {ConvertUnits.RainINToUser(data11.et.Value):F3} to ET");
									DoET(newET, recordTs);
								}
							}
							catch (Exception ex)
							{
								cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing Solar value on TxId {data11.tx_id}");
							}
						}
					...
					case 13: // Non-ISS data
						switch (sensorType)
						{
							case 56: // Soil + Leaf
								...
								{
									cumulus.LogExceptionMessage(ex, $"Error, DecodeHistoric, LeafWetness txid={data13.tx_id}, idx={idx}");
								}
								/*
								 * Soil Moisture
								 * Available fields
								...
								try
								{
									if (cumulus.WllExtraSoilMoistureTx1 == data13.tx_id)
									{
										...
										{
											DoSoilMoisture((int?)data13[idx], 1);
										}
									}
									if (cumulus.WllExtraSoilMoistureTx2 == data13.tx_id)
									{
										...
										{
											DoSoilMoisture((int?)data13[idx], 2);
										}
									}
									if (cumulus.WllExtraSoilMoistureTx3 == data13.tx_id)
									{
										...
										{
											DoSoilMoisture((int?)data13[idx], 3);
										}
									}
									if (cumulus.WllExtraSoilMoistureTx4 == data13.tx_id)
									{
										...
										{
											DoSoilMoisture((int?)data13[idx], 4);
										}
									}
								}
								catch (Exception ex)
								{
									cumulus.LogExceptionMessage(ex, $"Error, DecodeHistoric, SoilMoisture txid={data13.tx_id}, idx={idx}");
								}
								...
								{
									cumulus.LogExceptionMessage(ex, $"Error, DecodeHistoric, SoilTemp txid={data13.tx_id}, idx={idx}");
								}
							...
							case 242: // Baro
								...
								try
								{
									...
									{
										DoPressure(null, DateTime.MinValue);
										cumulus.LogMessage("WL.com historic: Warning, no valid Baro data (high)");
									}
									...
									if (data13baro.bar_absolute != null)
									{
										...
										AltimeterPressure = ConvertUnits.PressMBToUser(StationToAltimeter(ConvertUserPressureToHPa(StationPressure).Value, AltitudeM(cumulus.Altitude)));
									}
									else
									{
								...
								{
									cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing baro reading. Error");
								}
								break;
							...
							case 243: // Inside temp/hum
								...
								{
									cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing temp-in reading. Error");
								}
								...
								{
									DoIndoorHumidity(null);
									cumulus.LogExceptionMessage(ex, $"WLL current: Error processing humidity-in. Error");
								}
			...
			{
				cumulus.LogExceptionMessage(ex, $"Error, DecodeHistoric, DataType={dataType}, SensorType={sensorType}");
			}
		}
		...
		private void DecodeWlApiHealth(WlHistorySensor sensor, bool startingup)
		{
			...
			if (sensor.data_structure_type == 15)
			{
				...
				{
					cumulus.LogExceptionMessage(ex, $"WL.com historic: Error processing WLL health. Error:");
			...
			else if (sensor.data_structure_type == 11 || sensor.data_structure_type == 13)
			{
				...
				try
				{
					...
					if (sensor.data_structure_type == 11 && data.et != null)
					{
						...
						// This is unlike the existing VP2 when the ET is an annual running total
						// So we try and mimic the VP behaviour
						var newET = AnnualETTotal + ConvertUnits.RainINToUser(data.et.Value).Value;
						cumulus.LogDebugMessage($"WLL Health: Adding {ConvertUnits.RainINToUser(data.et.Value):F3} to ET");
						DoET(newET, DateTime.Now);
					}
				}
				catch (Exception ex)
				{
					cumulus.LogExceptionMessage(ex, "WLL Health: Error processing transmitter health. Error");
				}
			}
		}
		...
		private async Task GetWlHistoricHealth()
		{
			...
			try
			{
				...
				{
					cumulus.LogExceptionMessage(ex, "WLL Health: exception");
				}
		...
		private static void GetAvailableStationIds(bool logToConsole = false)
		{
			if (cumulus.WllApiKey == string.Empty || cumulus.WllApiSecret == string.Empty)
			{
			...
			try
			{
				...
				using (var response = Cumulus.MyHttpClient.SendAsync(request).Result)
				{
					...
					WeatherStation.LogRawExtraData(resp, false);
				}

				if (responseCode != 200)
				{
					var errObj = responseBody.FromJson<WlErrorResponse>();
					cumulus.LogWarningMessage($"WLLStations: WeatherLink API Error: {errObj.code} - {errObj.message}");
					return;
				}
			...
			{
				cumulus.LogExceptionMessage(ex, "WLLStations: WeatherLink API exception");
			}
			return;
		}
		...
		private void GetAvailableSensors()
		{
			...
			foreach (var sensor in sensorsObj.sensors)
			{
				...
				{
					cumulus.LogExceptionMessage(ex, "GetAvailableSensors: Processing sensors exception");
				}
			}
		}
		...
		private static async Task GetSystemStatus()
		{
			...
			try
			{
				...
				// we want to do this asynchronously
				using (var response = await Cumulus.MyHttpClient.GetAsync("https://0886445102835570.hostedstatus.com/1.0/status/600712dea9c1290530967bc6"))
				{
					responseBody = await response.Content.ReadAsStringAsync();
					responseCode = (int)response.StatusCode;
					cumulus.LogDebugMessage($"GetSystemStatus: WeatherLink.com system status Response code: {responseCode}");
					cumulus.LogDataMessage($"G etSystemStatus: WeatherLink.com system status Response: {responseBody}");
				}
			...
			{
				cumulus.LogExceptionMessage(ex, "GetSystemStatus: Exception");
			}
		...
		private class WllBroadcastRec
		{
			...
			public int rain_rate_last { get; set; }
		...
		private class WllCurrentType1
		{
			...
			public int? rain_size { get; set; }
			public int? rain_rate_last { get; set; }
			public int? rain_rate_hi { get; set; }
			public int? rainfall_last_15_min { get; set; }
			public int? rain_rate_hi_last_15_min { get; set; }
			public int? rainfall_last_60_min { get; set; }
			public int? rainfall_last_24_hr { get; set; }
			public int? rain_storm { get; set; }
			public long? rain_storm_start_at { get; set; }
