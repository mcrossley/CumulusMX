...
namespace CumulusMX
{
	internal class DavisStation : WeatherStation
	{
		...
		public DavisStation(Cumulus cumulus) : base(cumulus)
		{
			...
			// LOOP does not provide wind chill (loop2 does) so we always calculate WC in MX
		...
		private void DecodeReceptionStats(string recepStats)
		{
			try
			{
				...
				if (DavisTotalPacketsReceived < 0) DavisTotalPacketsReceived += 65536; // The console uses 16 bit signed variable to hold the value so it bit wraps
			...
			{
				cumulus.LogExceptionMessage(ex, "DecodeReceptionStats: Error");
			}
		}
		...
		private string GetFirmwareVersion()
		{
			...
			if (isSerial)
			{
				...
				if (WakeVP(comport))
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "GetFirmwareVersion: Error");
		...
		private void CheckLoggerInterval()
		{
			...
			LogRawStationData(BitConverter.ToString(readBuffer.Take(bytesRead).ToArray()), false);
		...
		private void SetLoggerInterval(int interval)
		{
			...
			if (isSerial)
			{
				...
				if (WakeVP(comport))
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "SetLoggerInterval: Error");
						awakeStopWatch.Stop();
					}
				}
			}
		...
		private string GetReceptionStats()
		{
			...
			if (isSerial)
			{
				...
				if (WakeVP(comport))
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "GetReceptionStats: Error");
			...
			response = response.Length > 10 ? response[0..^2] : "0 0 0 0 0";
		...
		private TcpClient OpenTcpPort()
		{
			...
			while (attempt < 5 && client == null && !stop)
			{
				...
				{
					cumulus.LogExceptionMessage(ex, "OpenTcpPort: Error");
				}
			}
		...
		*/
				...
				try
				{
					...
					if (isSerial)
					{
						...
						else
						{
							...
							{
								cumulus.LogExceptionMessage(ex, $"Failed to open the comm port ({cumulus.ComportName})");
							}
							if (comport == null || !comport.IsOpen)
							{
		...
		private void SendBarRead()
		{
			...
			if (isSerial)
			{
				...
				if (WakeVP(comport))
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "SendBarRead: Error");
		...
		private bool SendLoopCommand(SerialPort serialPort, string commandString)
		{
			...
			catch (Exception ex)
			{
				...
				cumulus.LogExceptionMessage(ex, "SendLoopCommand: Error sending LOOP command [" + commandString.Replace("\n", "") + "]");
		...
		private void GetAndProcessLoopData(int number)
		{
			...
			for (int i = 0; i < number; i++)
			{
				...
				else
				{
					...
					if (cumulus.DavisOptions.PeriodicDisconnectInterval > 0)
					{
						...
						if (min != previousMinuteDisconnect)
						{
							...
							{
								cumulus.LogExceptionMessage(ex, "LOOP: Periodic disconnect error");
							}
							finally
							{
				...
				LogRawStationData(BitConverter.ToString(loopString), false);
				...
				if (!(loopString[0] == 'L' && loopString[1] == 'O' && loopString[2] == 'O' && Convert.ToByte(loopString[4]) == 0))
				{
					...
					if (isSerial)
					{
						...
						{
							cumulus.LogExceptionMessage(ex, "LOOP: Error discarding bytes");
						}
					}
					else
					{
				...
				else
				{
					...
					cumulus.LogDebugMessage($"LOOP: Ignoring indoor humidity data. RH={loopData.OutsideHumidity} %.");
				}

				if ((loopData.OutsideHumidity >= 0) && (loopData.OutsideHumidity <= 100))
				{
					DoHumidity(loopData.OutsideHumidity, now);
				}
				else
				{
					DoHumidity(null, now);
				...
				if (loopData.CurrentWindSpeed < 200 && (loopData.AvgWindSpeed < 200 || loopData.AvgWindSpeed == 255))
				{
					...
					{
						cumulus.LogDebugMessage("LOOP: Wind direction = 0x7FFF = no reading, using null instead");
						winddir = null;
					}
					else if (winddir > 360)
					{
						cumulus.LogDebugMessage($"LOOP: Wind direction = {winddir}, using null instead");
						winddir = null;
					}

					if (loopData.AvgWindSpeed == 255)
					{
						// The console hasn't calculated an average yet, set to null
						avgwind = null;
					}
				...
				if (cumulus.ExtraDataLogging.SoilMoisture)
				{
					for (var j = 1; j <= 4; j++)
					{
						...
						int? moist = val >= 0 && val <= 250 ? val : null;
						DoSoilMoisture(moist, j);
					}
				}
				if (cumulus.ExtraDataLogging.SoilTemp)
				{
					for (var j = 1; j <= 4; j++)
					{
						var val = (int)loopData.GetPropValue("SoilTemp" + j);
						double? temp = val < 255 && val > 0 ? val : null;
						DoSoilTemp(temp, j);
					}
				}
				if (cumulus.ExtraDataLogging.LeafWetness)
				{
					for (var j = 1; j <= 4; j++)
					{
						var val = (int)loopData.GetPropValue("LeafWetness" + j);
						int? wet = val >= 0 && val < 16 ? val : null;
						DoLeafWetness(wet, j);
					}
				}
			...
			for (int i = 0; i < number; i++)
			{
				...
				else
				{
					...
					catch (System.IO.IOException ex)
					{
						...
						cumulus.LogExceptionMessage(ex, "LOOP2: Data: Error");
						cumulus.LogDebugMessage("LOOP2: Attempting to reconnect to logger");
						InitTCP();
						cumulus.LogDebugMessage("LOOP2: Reconnected to logger");
						return;
		...
		private void GetArchiveData()
		{
			...
			else
			{
				...
				{
					cumulus.LogExceptionMessage(ex, "GetArchiveData: Error sending LOOP command [DMPAFT]");
				...
				for (int p = 0; p < numPages; p++)
				{
					...
					do
					{
						...
						if (isSerial)
						{
							...
							LogRawStationData(BitConverter.ToString(buff), false);
					...
					for (int r = start; r < 5; r++)
					{
						...
						if (timestamp > lastDataReadTime)
						{
							...
							{
								DoHumidity(archiveData.OutsideHumidity, timestamp);
							}
							else
							{
				...
				while (retryCount < 1)
				{
					...
					while (passCount <= maxPasses)
					{
						...
						catch (System.IO.IOException ex)
						{
							...
							{
								cumulus.LogExceptionMessage(ex, "WakeVP: Error with TCP connection");
								cumulus.LogDebugMessage("WakeVP: Attempting reconnect to logger");
								InitTCP();
		...
		*/
		...
		private void InitSerial()
		{
			...
			do
			{
				...
				catch (Exception ex)
				{
					...
					cumulus.LogExceptionMessage(ex, "InitSerial: Error opening port");
				}
			...
			try
			{
				...
				do
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "InitSerial: Error");
					}
					tryCount++;
		...
		private void InitTCP()
		{
			...
			try
			{
				...
				do
				{
					...
					LogRawStationData(BitConverter.ToString(buffer1.Take(idx).ToArray()), false);
		...
		private bool WaitForOK(SerialPort serialPort)
		{
			...
			do
			{
				...
				{
					// Read the current character
					_ = readBuffer.Append((char)serialPort.ReadChar());
				}
				catch (TimeoutException)
				{
				...
				{
					cumulus.LogExceptionMessage(ex, "WaitForOK: Error");
			...
			do
			{
				...
				catch (System.IO.IOException ex)
				{
					if (ex.Message.Contains("did not properly respond after a period")) //TODO: Change to use error code
					{
						...
						LogRawStationData(BitConverter.ToString(Encoding.UTF8.GetBytes(readBuffer.ToString())), false);
						return false;
					}
		...
		private bool WaitForACK(SerialPort serialPort, int timeoutMs = -1)
		{
			...
			do
			{
				...
				{
					cumulus.LogExceptionMessage(ex, $"WaitForAck: {tryCount} Error");
		...
		private DateTime GetTime()
		{
			...
			if (isSerial)
			{
				...
				if (WakeVP(comport))
				{
					...
					{
						cumulus.LogExceptionMessage(ex, "getTime: Error");
						return DateTime.MinValue;
					}
				}
			}
		...
		private void SetTime()
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "SetTime: Error");
				return;
			}
