		...
		*/
		...
		private void ProcessPondPacket()
		{
			...
			if ((sensor > 1) && (sensor < 11))
			{
				...
				WMR200ExtraTempValues[sensor] = ConvertUnits.TempCToUser(num).Value;
		...
		private void ProcessTempPacket()
		{
			...
			if (sensor == cumulus.WMR200TempChannel)
			{
				...
				// outdoor hum
				DoHumidity(packetBuffer[5], DateTime.Now);
			...
			if ((sensor > 1) && (sensor < 11))
			{
				...
				num = (sign * ((packetBuffer[7] & 0xF) * 256 + packetBuffer[6])) / 10.0;
				WMR200ExtraDPValues[sensor] = ConvertUnits.TempCToUser(num).Value;
		...
		private void ProcessBaroPacket()
		{
			...
			double slp = ConvertUnits.PressMBToUser(num).Value;
		...
		private void SendReset()
		{
			...
			{
				cumulus.LogExceptionMessage(ex, "SendReset: Error");
			}
		}
