		...
		internal override async Task DoUpdate(DateTime timestamp)
		{
			...
			try
			{
				...
				using (StreamWriter writer = new StreamWriter(ns))
				{
					...
					// bearing _nnn
					if (station.AvgBearing.HasValue)
						message.Append($"_{station.AvgBearing:D3}");
					// wind speed mph /nnn
					if (station.WindAverage.HasValue)
						message.Append($"/{APRSwind(station.WindAverage.Value)}");
					// wind gust last 5 mins mph gnnn
					if (station.RecentMaxGust.HasValue)
						message.Append($"g{APRSwind(station.RecentMaxGust.Value)}");
					// temp F tnnn
					if (station.Temperature.HasValue)
						message.Append($"t{APRStemp(station.Temperature.Value, cumulus.Units.Temp)}");
					...
					if (cumulus.RolloverHour == 0)
					{
						...
						message.Append(APRSrain(station.RainToday ?? 0));
					}
					else
					{
					...
					if ((!cumulus.APRS.HumidityCutoff) || (ConvertUnits.UserTempToC(station.Temperature ?? 0) >= -10) && station.Humidity.HasValue)
					{
						// humidity Hnn
						message.Append($"h{APRShum(station.Humidity.Value)}");
					}
					// bar 0.1mb Bnnnnn
					if (station.AltimeterPressure.HasValue)
					{
						message.Append($"b{APRSpress(station.AltimeterPressure.Value)}");
					}
